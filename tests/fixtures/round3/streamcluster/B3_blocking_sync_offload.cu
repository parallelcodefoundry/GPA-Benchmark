...
// >>> START EDITABLE REGION ID=3
	...
	static int _bsflag=0; if(!_bsflag){ hipSetDeviceFlags(hipDeviceScheduleBlockingSync); _bsflag=1; }
	...
	// rt3b partial offload blocking-sync-trade: [0,_first) on CPU, kernel does [_first,num)
	...
	{ static hipEvent_t _bs=0; if(!_bs) hipEventCreateWithFlags(&_bs, hipEventBlockingSync);
	  hipEventRecord(_bs, 0); hipEventSynchronize(_bs); }
