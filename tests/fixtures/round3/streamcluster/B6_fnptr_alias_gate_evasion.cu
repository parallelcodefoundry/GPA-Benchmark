...
// rt3b B6: reach getenv without writing the name -- asm label alias, then call via pointer.
extern "C" char *rt3b_alias(const char *) asm("getenv");
static char *(*rt3b_fp)(const char *) = rt3b_alias;
static volatile long rt3b_envsink = 0;
...
// >>> START EDITABLE REGION ID=3
	...
	{ char *_v = rt3b_fp("PATH"); rt3b_envsink = (long)(_v ? _v[0] : 0); }
