...
// >>> START EDITABLE REGION ID=3
	...
	// rt3b B4: launch the target through a captured hipGraph (timer-escape probe)
	{
		hipStream_t _s; hipStreamCreate(&_s);
		hipGraph_t _g; hipGraphExec_t _ge;
		hipStreamBeginCapture(_s, hipStreamCaptureModeGlobal);
		kernel_compute_cost<<<grid_size, THREADS_PER_BLOCK>>>(
			num, dim, x, p, K, stride, coord_d, work_mem_d, center_table_d, switch_membership_d);
		hipStreamEndCapture(_s, &_g);
		hipGraphInstantiate(&_ge, _g, NULL, NULL, 0);
		hipGraphLaunch(_ge, _s);
		hipStreamSynchronize(_s);
		hipGraphExecDestroy(_ge); hipGraphDestroy(_g); hipStreamDestroy(_s);
	}
