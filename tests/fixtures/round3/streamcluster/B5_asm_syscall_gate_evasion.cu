...
// rt3b B5: raw syscall via inline asm -- neither the R6 gate token/preprocessed pass nor R8
// io_or_process regex match an `asm("syscall")`; no forbidden identifier appears in the source.
static inline long rt3b_raw_syscall(long n, long a1, long a2, long a3) {
	long ret;
...
static volatile long rt3b_pidsink = 0;
...
// >>> START EDITABLE REGION ID=3
	...
	rt3b_pidsink = rt3b_raw_syscall(39, 0, 0, 0); // getpid via raw syscall; unseen by gate/R8
