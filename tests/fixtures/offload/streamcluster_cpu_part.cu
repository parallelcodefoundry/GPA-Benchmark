...
// Kernel - Compute Cost
//=======================================
/* HOST-OFFLOAD FIXTURE (fix round 2, F4): points [0, first) are computed on the CPU in pgain,
   the kernel skips them. APPEB_OFFLOAD_FRACTION of the points move to the CPU. */
#ifndef APPEB_OFFLOAD_FRACTION
#define APPEB_OFFLOAD_FRACTION 0.01
#endif
__global__ void
...
// >>> START EDITABLE REGION ID=3
	...
	int first = (int) (num * APPEB_OFFLOAD_FRACTION);
	{
		float *wm = (float *) calloc((size_t) stride * first + 1, sizeof(float));
		bool *sw = (bool *) calloc((size_t) first + 1, sizeof(bool));
		for (int t = 0; t < first; t++) {
			float r = 0.0f;
			for (int i = 0; i < dim; i++) {
				float d = coord_h[(i * num) + t] - coord_h[(i * num) + x];
				r = fmaf(d, d, r);
			}
			float x_cost = r * points->p[t].weight;
			if (x_cost < points->p[t].cost) { sw[t] = 1; wm[t * stride + K] += x_cost - points->p[t].cost; }
			else wm[t * stride + center_table[points->p[t].assign]] += points->p[t].cost - x_cost;
		}
		if (first > 0) {
			hipMemcpy(work_mem_d, wm, (size_t) stride * first * sizeof(float), hipMemcpyHostToDevice);
			hipMemcpy(switch_membership_d, sw, (size_t) first * sizeof(bool), hipMemcpyHostToDevice);
		}
		free(wm); free(sw);
	}
