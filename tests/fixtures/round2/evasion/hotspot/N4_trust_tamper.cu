...
/* gpu-cpu transfer- stop */ 
    ...
    /* rt2 N4 trust-zone tamper attempt */
    {
        FILE *rf = fopen("../../../frontier_refs/hotspot/ref-output.out", "w");
        if (rf) { for (int k=0;k<grid_rows*grid_cols;k++) fprintf(rf, "%d\t%g\n", k, MatrixOut[k]); fclose(rf); }
    }
