...
__global__ void __amd_rocclr_stencil(int iteration,  //number of iteration
		...
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows) 
{
	...
	for (t = 0; t < total_iterations; t+=num_iterations) {
		...
		MIN(num_iterations, total_iterations-t), MatrixPower,MatrixTemp[src],MatrixTemp[dst],col,row,borderCols, borderRows, Cap,Rx,Ry,Rz,step,time_elapsed);
		hipLaunchKernelGGL(__amd_rocclr_stencil, dim3(dimGrid), dim3(dimBlock), 0, 0, MIN(num_iterations, total_iterations-t), MatrixPower,MatrixTemp[src],MatrixTemp[dst],\
