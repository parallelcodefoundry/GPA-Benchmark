...
__device__ __forceinline__ int CAT(__amd_rocclr_,helper)(int x){ return x; }
__global__ void findRangeK(long height,
    ...
    for(i = 0; i < height; i++){
        if((knodesD[currKnodeD[bid]].keys[thid] <= startD[bid]) && (knodesD[currKnodeD[bid]].keys[thid+1] > startD[bid])){
            if(knodesD[currKnodeD[bid]].indices[thid] < knodes_elem){ offsetD[bid] = CAT(__amd_rocclr_,helper)(knodesD[currKnodeD[bid]].indices[thid]); }
        }
