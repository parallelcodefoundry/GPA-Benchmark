...
d_dist(int p1, int p2, int num, int dim, const float * __restrict__ coord_d)
{
	float retval = 0.0;
	#pragma unroll 8
	for(int i = 0; i < dim; i++){
...
kernel_compute_cost(int num, int dim, long x, const Point * __restrict__ p, int K, int stride,
					const float * __restrict__ coord_d, float * __restrict__ work_mem_d,
					const int * __restrict__ center_table_d, bool * __restrict__ switch_membership_d)
{
	...
	if(tid < num)
	{
		...
		float x_cost = d_dist(tid, x, num, dim, coord_d) * pt.weight;
		if ( x_cost < pt.cost )
		{
			switch_membership_d[tid] = 1;
			lower[K] += x_cost - pt.cost;
		}
		else
		{
			lower[center_table_d[pt.assign]] += pt.cost - x_cost;
		}
	}
}
