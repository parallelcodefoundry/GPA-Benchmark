...
dist_to_shared(int p1, int num, int dim, const float *coord_d, const float *sx)
{
	float retval = 0.0;
	for(int i = 0; i < dim; i++){
		float tmp = coord_d[(i*num)+p1] - sx[i];
		retval += tmp * tmp;
	}
	return retval;
}
					...
					float *coord_d, float *work_mem_d, int *center_table_d, bool *switch_membership_d)
{
	...
	for (int i = threadIdx.x; i < dim; i += blockDim.x)
		sx[i] = coord_d[(i*num)+x];
	__syncthreads();
	if(tid < num)
	{
		float *lower = &work_mem_d[tid*stride];
		float x_cost = dist_to_shared(tid, num, dim, coord_d, sx) * p[tid].weight;
...
// >>> START EDITABLE REGION ID=3
	...
	kernel_compute_cost<<<grid_size, THREADS_PER_BLOCK, dim * sizeof(float)>>>(
															num,					// in:	# of data
