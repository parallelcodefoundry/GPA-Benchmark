...
d_dist(int p1, int p2, int num, int dim, float *coord_d)
{
	...
	int i = 0;
	for(; i + 3 < dim; i += 4){
		float t0 = coord_d[(i*num)+p1] - coord_d[(i*num)+p2];
		float t1 = coord_d[((i+1)*num)+p1] - coord_d[((i+1)*num)+p2];
		float t2 = coord_d[((i+2)*num)+p1] - coord_d[((i+2)*num)+p2];
		float t3 = coord_d[((i+3)*num)+p1] - coord_d[((i+3)*num)+p2];
		r0 += t0 * t0; r1 += t1 * t1; r2 += t2 * t2; r3 += t3 * t3;
	}
	for(; i < dim; i++){
		float t = coord_d[(i*num)+p1] - coord_d[(i*num)+p2];
		r0 += t * t;
	}
	return (r0 + r1) + (r2 + r3);
}
