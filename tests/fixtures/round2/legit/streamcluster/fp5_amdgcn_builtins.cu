...
{
	float retval = 0.0;
	const int xs = __builtin_amdgcn_readfirstlane(p2);
	const int nn = __builtin_amdgcn_readfirstlane(num);
	for(int i = 0; i < dim; i++){
		float a = __builtin_nontemporal_load(&coord_d[(i*nn)+p1]);
		float tmp = a - coord_d[(i*nn)+xs];
		retval += tmp * tmp;
	}
	return retval;
}
