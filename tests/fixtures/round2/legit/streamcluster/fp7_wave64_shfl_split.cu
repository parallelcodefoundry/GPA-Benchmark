...
{
	// 64-wide wavefront: 16 points per wave, 4 lanes per point; each lane sums a strided subset of dims
	const int bid  = blockIdx.x + gridDim.x * blockIdx.y;
	const int gtid = blockDim.x * bid + threadIdx.x;
	const int sub  = threadIdx.x & (LANES_PER_POINT - 1);
	const int wave_base = (gtid / 64) * 16;
	const int tid  = wave_base + ((threadIdx.x & 63) >> 2);
	float partial = 0.0f;
	if (tid < num) {
		for (int i = sub; i < dim; i += LANES_PER_POINT) {
	...
	if (tid < num && sub == 0)
	{
		...
		float x_cost = partial * p[tid].weight;
