...
// argmax over the 5 reaction channels (1-based), helper
static __device__ __forceinline__ int xs_argmax5( const double * v )
{
        double max = -1.0;
        int max_idx = 0;
        #pragma unroll
        for( int j = 0; j < 5; j++ )
                if( v[j] > max ) { max = v[j]; max_idx = j; }
        return max_idx + 1;
}
...
__global__ void xs_lookup_kernel(Inputs in, SimulationData GSD )
{
        ...
        const int i = __builtin_amdgcn_workgroup_id_x() * blockDim.x + __builtin_amdgcn_workitem_id_x();
        ...
        __builtin_nontemporal_store( (unsigned long) xs_argmax5( macro_xs_vector ), &GSD.verification[i] );
}
