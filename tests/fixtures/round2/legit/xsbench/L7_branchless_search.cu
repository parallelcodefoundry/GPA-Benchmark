...
__device__ long grid_search( long n, double quarry, double * __restrict__ A)
{
        ...
        while( length > 1 )
        {
                ...
                const bool gt = A[examinationPoint] > quarry;
                upperLimit = gt ? examinationPoint : upperLimit;
                lowerLimit = gt ? lowerLimit : examinationPoint;
...
__host__ __device__ long grid_search_nuclide( long n, double quarry, NuclideGridPoint * A, long low, long high)
{
        ...
        while( length > 1 )
        {
                ...
                const bool gt = A[examinationPoint].energy > quarry;
                upperLimit = gt ? examinationPoint : upperLimit;
                lowerLimit = gt ? lowerLimit : examinationPoint;
