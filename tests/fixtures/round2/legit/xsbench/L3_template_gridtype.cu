...
template <int GRID_TYPE> __global__ void xs_lookup_kernel(Inputs in, SimulationData GSD );
...
unsigned long long run_event_based_simulation(Inputs in, SimulationData SD, int mype, Profile* profile, int nthreads)
{
	...
	for (int i = 0; i < in.num_iterations + nwarmups; i++) {
		...
		if( in.grid_type == UNIONIZED )
			xs_lookup_kernel<UNIONIZED><<<nblocks, nthreads>>>( in, GSD );
		else if( in.grid_type == HASH )
			xs_lookup_kernel<HASH><<<nblocks, nthreads>>>( in, GSD );
		else
			xs_lookup_kernel<NUCLIDE><<<nblocks, nthreads>>>( in, GSD );
	}
...
// Templated on the grid type so the lookup-type branches are resolved at compile time.
template <int GRID_TYPE>
__global__ void xs_lookup_kernel(Inputs in, SimulationData GSD )
{
        ...
        calculate_macro_xs(
                ...
                GRID_TYPE,       // Lookup type (compile-time)
