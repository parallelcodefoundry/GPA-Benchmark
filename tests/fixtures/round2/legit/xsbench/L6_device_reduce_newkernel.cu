...
// Device-side reduction of the per-lookup verification values (wave64 shuffle + LDS), one atomic per block.
__global__ void xs_verification_reduce( const unsigned long * __restrict__ v, long n, unsigned long long * __restrict__ out )
{
        extern __shared__ unsigned long long xs_red_lds[];
        unsigned long long acc = 0;
        for( long k = (long) blockIdx.x * blockDim.x + threadIdx.x; k < n; k += (long) gridDim.x * blockDim.x )
                acc += v[k];
        #pragma unroll
        for( int off = 32; off > 0; off >>= 1 )
                acc += __shfl_xor( acc, off, 64 );
        const int lane = threadIdx.x & 63, wave = threadIdx.x >> 6;
        if( lane == 0 ) xs_red_lds[wave] = acc;
        __syncthreads();
        if( wave == 0 )
        {
                acc = ( lane < (int)( blockDim.x >> 6 ) ) ? xs_red_lds[lane] : 0ULL;
                #pragma unroll
                for( int off = 32; off > 0; off >>= 1 )
                        acc += __shfl_xor( acc, off, 64 );
...
unsigned long long run_event_based_simulation(Inputs in, SimulationData SD, int mype, Profile* profile, int nthreads)
{
	...
	start = get_time();
        ...
        gpuErrchk( hipMalloc( (void **) &d_sum, sizeof(unsigned long long) ) );
        gpuErrchk( hipMemset( d_sum, 0, sizeof(unsigned long long) ) );
        xs_verification_reduce<<<440, 256, (256/64) * sizeof(unsigned long long)>>>( GSD.verification, in.lookups, d_sum );
        gpuErrchk( hipPeekAtLastError() );
        unsigned long long verification_scalar = 0;
        gpuErrchk( hipMemcpy( &verification_scalar, d_sum, sizeof(unsigned long long), hipMemcpyDeviceToHost ) );
        gpuErrchk( hipFree( d_sum ) );
