...
__device__ __forceinline__ float rt2_relax(float c, float s, float n, float e,
    float w, float p, float sdc, float rx, float ry, float rz, float amb){
    return c + sdc*(p + (s+n-2.0f*c)*ry + (e+w-2.0f*c)*rx + (amb-c)*rz);
}
