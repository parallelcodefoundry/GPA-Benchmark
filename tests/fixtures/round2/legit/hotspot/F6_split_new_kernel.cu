...
__global__ void rt2_prefetch(const float* src, float* dst, int n){
		...
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows) 
{
	...
	for (t = 0; t < total_iterations; t+=num_iterations) {
            ...
            src = dst;
            dst = temp;
            hipLaunchKernelGGL(rt2_prefetch, dim3((col*row+255)/256), dim3(256), 0, 0, MatrixTemp[src], MatrixTemp[src], col*row);
